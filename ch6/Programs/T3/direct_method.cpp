...
//    }
    for(int i = 0; i<px_ref.size(); ++i)
    {
        ...
        auto p_ref = px_ref[i];
        if(p_cur[0]>0 && p_cur[1]>0)  //大于0说明投影后未出界
        {
            cv::circle(img2_show, cv::Point2f(p_cur[0], p_cur[1]), 2, cv::Scalar(0, 250, 0), 2);
            cv::line(img2_show, cv::Point2f(p_ref[0], p_ref[1]), cv::Point2f(p_cur[0], p_cur[1]),
                     cv::Scalar(0, 250, 0));
        }
//        cv::rectangle(img2_show, cv::Point2f(pcur[0] - 2, pcur[1] - 2), cv::Point2f(pcur[0] + 2, pcur[1] + 2), cv::Scalar(0, 250, 0));
//        cv::line(img2_show, cv::Point2f(pcur[0], pcur[1]), cv::Point2f(pref[0], pref[1]), cv::Scalar(0, 250, 0));
    }
