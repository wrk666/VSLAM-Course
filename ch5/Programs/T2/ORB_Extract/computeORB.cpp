...
//C++11标准中的一些互斥访问的类与方法等(在多线程中往vector有序容器中插入数据时顺序会被破坏，需要加锁以保证顺序)
#include <mutex>
 ...
 */
typedef vector<bool> DescType; // type of descriptor, 256 bool
...
//  cv::waitKey(0);
    ...
    // compute angle for each keypoint  lambda表达式，传引用的形式调用
...
//    cout<<"again compute DescMT!"<<endl;
    evaluate_and_call([&]() { computeORBDescMT(first_image, keypoints, descriptors_mt); }, "compute orb descriptor mt", 1);
...
//  cv::waitKey(0);
    ...
    cout<<"the second Image ORB calculating..."<<endl;
...
////// compute the angle   计算FAST角点的方向信息
//void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
...
//        //如果在左右边界或者4个角上都取不了16*16的块，那就认为不是好角点
//        if(kp.pt.x<half_patch_size || kp.pt.y<half_patch_size || kp.pt.x>=image.cols-half_patch_size || kp.pt.y >= image.cols - half_patch_size)
//        {
//            ++bad_points;
//            //本来这里对于描述子应该是空，但是因为这里仅仅是计算角度，所以没有
////            descriptors.push_back({});
//            continue;
//        }
//        float m01=0, m10=0;
//        for(int dx=-half_patch_size; dx < half_patch_size; ++dx)  //x方向遍历16个点(右)
//            for(int dy=-half_patch_size; dy < half_patch_size; ++dy)  //y方向遍历16个点(下)
//            {
//                uchar pixel = image.at<uchar>(kp.pt.y+dy, kp.pt.x+dx);  //指针移动访问对应的像素亮度
//                m10 +=dx * pixel;
//                m01 +=dy * pixel;
//            }
//        //计算角度（弧度制）并转换为角度
...
//                          float m01=0, m10=0;
//                          for(int i=u-half_patch_size; i < u + half_patch_size; ++i)  //x方向遍历16个点(右)
//                              for(int j=-half_patch_size; j < v + half_patch_size; ++j)  //y方向遍历16个点(下)
//                              {
////                                  uchar pixel = image.at<uchar>(kp.pt.y+dy, kp.pt.x+dx);  //指针移动访问对应的像素亮度
//                                  m10 +=i * image.at<uchar>(j, i);
//                                  m01 +=j * image.at<uchar>(j, i);
...
//// compute the descriptor  描述子个数和特征点个数相同
...
//      DescType d(256, false);   //256位的vector<bool>，书上定义的是8个unsigned int（32位）的数据，左移来置位，速度快，但是缺点是不好访问，得先访问每个unsigned int数据，在访问对应的位，不过快是优势
//      if (kp.pt.x < half_patch_size || kp.pt.y < half_patch_size || kp.pt.x >= image.cols - half_patch_size ||
//          kp.pt.y >= image.cols - half_patch_size)
//      {
//          ++bad_points;
//          desc.push_back({});
//          continue;
...
//
//        //使用std::sin和算梯度::cos需要先将角度转换为弧度 *pi/180
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2,
  ...
  // find matches between desc1 and desc2.  核心方法是按位异或^，但是这里是按照vector<bool>的每一位来判断
    for(size_t i1=0; i1<desc1.size(); ++i1)
    {
        ...
        cv::DMatch m((int)i1, 0, 256);  //DMatch(int _queryIdx, int _trainIdx, float _distance)：_queryIdx：为第i1个点进行匹配; _trainIdx:目标点匹配点为第0个;_distance:最小距离初始化为256
...
void computeORBDesc_1(const cv::Mat &image, vector<cv::KeyPoint> &keypoints,
                    vector<DescType> &desc) {
    for (auto &kp : keypoints) {
        DescType d(256, false); //256个0,用于存放描述子
        for (int i = 0; i < 256; i++) {
